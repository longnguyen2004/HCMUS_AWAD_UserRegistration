...
export default function BusEditor({
  ...
  const handleUpdateSeatLabel = (seatId: string, newLabel: string) => {
    setEditingBus({
      ...editingBus,
      seats: editingBus.seats.map((seat) =>
        seat.id === seatId ? { ...seat, seatNumber: newLabel } : seat,
      ),
    });
      ...
      <CardContent className="space-y-4">
                ...
                {editingSeatLabel?.seatId === seat.id ? (
                  <div className="flex flex-col gap-1">
                    <Input
                      ...
                      onBlur={() =>
                        handleUpdateSeatLabel(
                          seat.id,
                          editingSeatLabel.seatNumber,
                        )
                      }
                      onKeyDown={(e) => {
                        if (e.key === "Enter") {
        ...
        {isEditing && (
          <p className="text-xs text-muted-foreground">
            Click on any seat to edit its label
          </p>
        )}
